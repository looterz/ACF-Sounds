...
if( Bullet and !self.Cracked and ACF.SonicCracks[ Bullet.Crate:GetNetworkedString( "WireName" ) ] != nil ) then
  if( self:GetPos():Distance( LocalPlayer():GetPos() ) < 160 and self:GetPos():Distance( Bullet.Crate:GetPos() ) > 512 ) then
    ACF_SonicCrack( Bullet.Crate:GetNetworkedString( "WireName" ), self:GetPos() );
    self.Cracked = true;
  end
end
